...
<p>처음에는 자릿수에 따라서 3,6,9를 찾아 카운트할 생각이였다. 그래서 String.valueOf()로 변환한 다음에 for문을 이용해 chatAt()으로 한자리씩 읽고 했으나 케이스 문제는 통과했지만 제출 케이스들에서 점수가 57.1점이 나왔다..결국 서칭을 통해서 String[] 배열로 만든 후, 하나씩 비교하며 카운트했다.</p>
