...
<p>
계산하기 위해서는 두 개의 숫자가 필요하고 하나의 연산기호가 필요하다.
처음과 끝은 무조건 숫자이다.
두 수 사이에 중간은 무조건 연산기호이다.
위의 명제들을 세워서 식을 만들었지만 연습케이스는 통과했지만, 제출케이스는 30점이 나와 통과하지 못했다..
이유는 배열의 처음 숫자 값을 넣지 않고 시작했기 때문에 그랬다..
</p>
